      ...
      <main>
        <section className="relative mx-auto grid max-w-[1440px] items-center gap-10 px-5 pb-16 pt-12 md:px-10 md:pb-28 md:pt-20 lg:grid-cols-[1.03fr_.97fr] lg:px-14">
          <div className="relative z-10 mama-rise">
            ...
            <p className="mt-7 max-w-lg text-base leading-7 text-muted-foreground md:text-lg">A calm, voice-first bridge for pregnancy and postpartum care. Speak naturally in English, Nigerian Pidgin, Yoruba, or Igbo.</p>
        ...
        <section className="mx-auto grid max-w-[1440px] gap-8 px-5 py-16 md:grid-cols-[.8fr_1.2fr] md:px-10 md:py-24 lg:px-14">
          ...
          <div className="grid gap-3 self-end sm:grid-cols-2">
            {['English', 'Nigerian Pidgin', 'Yorùbá', 'Igbo'].map((lang, i) => <button onClick={() => start(lang)} key={lang} data-testid={`button-language-${i}`} className="mama-focus flex items-center justify-between rounded-2xl border border-border bg-card p-5 text-left transition hover:-translate-y-1 hover:border-primary"><span className="font-semibold">{lang}</span><ArrowRight size={17} className="text-muted-foreground" /></button>)}
          </div>
        </section>
        <footer className="mx-auto flex max-w-[1440px] flex-col gap-4 border-t border-border px-5 py-8 text-xs text-muted-foreground md:flex-row md:items-center md:justify-between md:px-14"><span>© 2025 MAMA · a prototype for safer maternal pathways</span><div className="flex gap-5"><Link href="/judge" data-testid="link-judge-footer">Competition demo</Link><Link href="/responsible-ai" data-testid="link-ai-footer">Responsible AI</Link></div></footer>
      </main>
    </div>
  );
}
    ...
    <div className="mama-noise min-h-[100dvh] bg-background text-foreground">
      ...
      <main className="mx-auto grid max-w-[1440px] gap-5 px-4 py-5 md:px-8 lg:grid-cols-[minmax(0,1.25fr)_360px]">
        ...
        <aside className="space-y-4">
          <section className="rounded-[24px] border border-border bg-card p-5"><div className="flex items-center justify-between"><div><p className="font-mono text-[10px] uppercase tracking-[.16em] text-muted-foreground">Step 02 / structure</p><h2 className="mt-1 font-[var(--app-font-serif)] text-xl font-extrabold tracking-[-.04em]">What MAMA heard</h2></div><BrainCircuit size={20} className="text-accent" /></div><div className="mt-5 space-y-3">{[['Intent', structure?.intent], ['Pregnancy', structure?.pregnancyStatus], ['When', structure?.duration], ['Severity', structure?.severity]].map(([label, value]) => <div key={label} className="flex items-start justify-between gap-3 border-b border-border pb-3 last:border-0 last:pb-0"><span className="text-xs text-muted-foreground">{label}</span><span data-testid={`text-structure-${String(label).toLowerCase()}`} className="text-right text-xs font-bold">{value || <span className="font-normal text-muted-foreground">Waiting to hear</span>}</span></div>)}</div>{structure?.symptoms?.length ? <div className="mt-4 flex flex-wrap gap-1.5">{structure.symptoms.map((item) => <Pill tone="teal" key={item}>{item}</Pill>)}</div> : null}<Button variant="quiet" onClick={runAnalysis} disabled={analyze.isPending || !messages.length} className="mt-5 w-full" testId="button-analyze">{analyze.isPending ? <><RefreshCw size={16} className="animate-spin" /> Checking safely</> : <><ShieldCheck size={16} /> Run safety check</>}</Button></section>
